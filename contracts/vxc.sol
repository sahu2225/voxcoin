...
contract VoxCoin is Context, BEP20 {
    address public taxWallet;
    uint256 public taxBps = 100; // 0.25% = 25 basis points

    constructor(address _taxWallet) BEP20("VoxCoin", "VXC") {
    ...
    //  Update Tax in Basis Points (Max 1000 = 10%)
    function setTaxBps(uint256 _bps) external onlyOwner {
        require(_bps <= 2500, "Max 25% tax");
        taxBps = _bps;
    }

    //  Override Transfer to Add 0.25% Tax
    function _transfer(
        address sender,
        address recipient,
        uint256 amount
        ...
        uint256 taxAmount = (amount * taxBps) / 10000; // using basis points
